...
const applyFilter = (filters: Filteration) => {
  ...
  const filteredSalesOrders = initialSalesOrderList.value.filter((salesOrder) => {
    ...
    const isWithinDateRange =
      ...
      (isValidFromDate && isValidToDate && createdDate >= fromDate && createdDate <= toDate) ||
      (isValidFromDate && !isValidToDate && createdDate >= fromDate) ||
      (!isValidFromDate && isValidToDate && createdDate <= toDate)

    // filter by customer name
    const isMatchingCustomerName = salesOrder.customerName.includes(filters.customerName)
    ...
    // filter by country
    const isMatchingCountry = filters.country === '' || salesOrder.country === filters.country
